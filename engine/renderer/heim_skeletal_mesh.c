...
HeimSkeletalMesh* heim_skeletal_mesh_create(HeimSkeletalMeshVertex* vertices, uint32_t* indices) {
    ...
    glBufferData(GL_ARRAY_BUFFER, heim_vector_length(mesh->vertices) * sizeof(HeimSkeletalMeshVertex), mesh->vertices,
                 GL_STATIC_DRAW);
    ...
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(HeimSkeletalMeshVertex),
                          (void*)offsetof(HeimSkeletalMeshVertex, normal));
    glEnableVertexAttribArray(2);
    glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(HeimSkeletalMeshVertex),
                          (void*)offsetof(HeimSkeletalMeshVertex, tex_coords));

    glEnableVertexAttribArray(3);
    glVertexAttribIPointer(3, 4, GL_INT, sizeof(HeimSkeletalMeshVertex), (void*)offsetof(HeimSkeletalMeshVertex, ids));
    glEnableVertexAttribArray(4);
    glVertexAttribPointer(4, 4, GL_FLOAT, GL_FALSE, sizeof(HeimSkeletalMeshVertex),
                          (void*)offsetof(HeimSkeletalMeshVertex, weights));

    glBindVertexArray(0);
    return mesh;
}

void heim_skeletal_mesh_destroy(HeimSkeletalMesh* mesh) { HEIM_FREE(mesh, HEIM_MEMORY_TYPE_RENDERER); }
