...
HeimAnimator* heim_animator_init(const char* path, HeimSkeletalModel* model) {
    ...
    for (uint32_t i = 0; i < scene->mNumAnimations; i++) {
        ...
        HeimAnimation* animation = heim_animation_init(scene, scene->mAnimations[i], model);
...
void heim_animator_free(HeimAnimator* animator) {
    for (size_t i = 0; i < heim_vector_length(animator->animations); i++) {
        heim_animation_free(animator->animations[i]);
    }
...
void heim_animator_update(HeimAnimator* animator, float dt) {
    ...
    if (animator->current_animation) {
        ...
        heim_animator_calculate_bone_transform(animator, &animator->current_animation->root_node, identity);
    }
}
...
void heim_animator_calculate_bone_transform(HeimAnimator* animator, const struct assimp_node_data_t* node, HeimMat4 parent_transform) {
    ...
    HeimVec3f scale;
    if (bone) {
        heim_bone_update(bone, animator->current_time);
        position = bone->local_position;
        rotation = bone->local_rotation;
        scale = bone->local_scale;
        if (animator->blend_animation) {
            HeimBone* blend_bone = heim_animation_find_bone(animator->blend_animation, node_name);
            if (blend_bone) {
                heim_bone_update(blend_bone, animator->blend_time);
    ...
    for (int32_t i = 0; i < node->children_count; i++) {
        heim_animator_calculate_bone_transform(animator, &node->children[i], global_transform);
    }
}
