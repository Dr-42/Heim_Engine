...
void* _heim_vector_resize(void* array) {
    ...
    void* temp = _heim_vector_create((HEIM_VECTOR_RESIZE_FACTOR * heim_vector_capacity(array)), stride);
...
void* _heim_vector_pop_at(void* array, uint64_t index, void* dest) {
    ...
    if (index != length - 1) {
        heim_memcpy((void*)(addr + (index * stride)), (void*)(addr + ((index + 1) * stride)), stride * (length - index));
    }
...
void* _heim_vector_insert_at(void* array, uint64_t index, void* value_ptr) {
    ...
    if (index != length - 1) {
        heim_memcpy((void*)(addr + ((index + 1) * stride)), (void*)(addr + (index * stride)), stride * (length - index));
    }
